class Mission < ActiveRecord::Base
  ...
  def status_description
    return I18n.t('missions.status.available_to_accept') if assigned_to.nil?
    case status
    when 1 then assigned_to.nickname + I18n.t('missions.status.ongoing')
    when 2 then assigned_to.nickname + I18n.t("missions.status.proposed")
    when 3 then assigned_to.nickname + I18n.t("missions.status.finished")
  ...
  def can_accept?(current_user)
    status == 0 && assigned_to.nil? && creator != current_user
  end

  def can_finish?(current_user)
    status == 1 && assigned_to == current_user
  end
